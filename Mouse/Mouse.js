	...
	function Run()
	{
        ...
        REFRESH_INTERVAL = setInterval(function() { DrawAnimatedMouse(300, 300, 300, 5 * Math.PI / 3, 0, ctx) },
                                                    REFRESH_RATE);
	}

    function DrawAnimatedMouse(x, y, length, direction, animationIndex, ctx)
    {
        clearInterval(REFRESH_INTERVAL);

        DrawMouse(x, y, length, direction, animationIndex, ctx);

        direction = direction + Math.PI / 40;

        animationIndex = animationIndex >= MOUSE_ANIMATION_STEPS - 1 ? 0 : animationIndex + 1;

        REFRESH_INTERVAL = setInterval(function() { DrawAnimatedMouse(300, 300, 300, direction, animationIndex, ctx); },
                                                    REFRESH_RATE);
    }
