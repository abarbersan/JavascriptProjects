...
function MoveBall(bList) {
    ...
    if (newAngle == randomAngle) {
        ...
        //var collisionResultAngle = GetBallCollisionAngle(newBall);
...
function DrawShape(shape) {
    ...
    for (var i = 0; i < shapeList.length; i++) {
        ...
        var offsetColor = RGBToHex(Math.floor(color.r * ((10 - offSet) / 10)),
										Math.floor(color.g * ((10 - offSet) / 10)),
										Math.floor(color.b * ((10 - offSet) / 10)));

        if (shape.type == "circle") {
            DrawBall(shapeList[i], offsetColor);
        } else if (shape.type == "triangle") {
